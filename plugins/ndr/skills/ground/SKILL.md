...
description: >
  ...
  plugin. Resolves structured scopes with direct `ndr` CLI calls (the
  CLI walks supersession in-process); free-text scopes dispatch
  `@ndr-reader`. Returns a compact brief plus the `ndr:` reference
  strings the caller can paste into prompts, code comments, or commit
  messages.
