...
| starts with `#` | slug | `ndr resolve '#<slug>'` |
| exactly one `/`, no whitespace | area/topic | `ndr resolve '<area>/<topic>'` (add `--verbose` for full briefs) |
...
Markdown remains canonical for NDRs; swamp migration paused (0102-markdown-remains-canonical-for-ndrs-swamp-migration-paused)
  area: substrate, topic: substrate, decision: 2026-05-28
  reversibility: easy
...
```
**Current decisions on "architecture/repo-shape":**
...
- The user wants a list of all decisions — `ndr current` (optionally
  `--area` / `--topic`), or the "Current Decisions" Obsidian Base.
