---
name: ndr-bootstrap
description: One-time install of ndr's seed content into a ledger directory. Copies the seed decision atoms (A–H meta-chain at 0001-0008 plus reference-addressability resolution at 0049-0051) and the initial `labels.yaml` taxonomy file into the ledger named by the argument or `$NDR_LEDGER`. Idempotent — re-runs skip targets that already exist. Invoke explicitly with `/ndr-bootstrap <ledger-dir>` once per machine after installing the ndr plugin.
