...
| `binds` | list[string] | no | Repo-relative glob patterns (`Bun.Glob` syntax) naming the code this decision governs. Consumed by `drift-check` (diff -> candidate atoms) and `/ground` (rank heads by overlap). Advisory routing signal, never an exclusive filter. Convention: bind directories/layers, not files — but as a **glob with `/**`** (`src/auth/**`, `plugins/ndr/agents/**`); a bare directory path (`src/auth`) matches no file under `Bun.Glob` and `ndr doctor` flags it `binds_matches_nothing`. Capture validates glob *syntax* only, not that it matches. |
